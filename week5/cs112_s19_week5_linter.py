_module = 'cs112_s19_week5_linter.py version 1.0'
...
# bannedTokens list for week 3
_bannedTokens = (
        'as,except,finally,' +
        'global,nonlocal,raise,' +
        'try,yield,' +
        '__import__,ascii,bin,bytearray,bytes,callable,' +
        'classmethod,compile,delattr,dict,dir,enumerate,' +
        'eval,exec,filter,format,frozenset,getattr,globals,' +
        'hasattr,hash,help,hex,id,issubclass,iter,' +
        'locals,map,memoryview,next,oct,' +
        'open,property,set,' +
...
'''
    ...
    def lint(self):
        print('Linting with %s in Python %s... '% (_module, platform.python_version()), end='')
        self.errors = [ ]
        if (self.code == None):
